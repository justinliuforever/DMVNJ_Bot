...
async function waitForTarget(){
    ...
    while (check) {
        try {  
            var data = fs.readFileSync('./discord_bot/init_target.txt', 'utf8');
            console.log('wait...'); 
            links = data.toString();  
        } catch(e) {
...
await page.type("#phone","1214101569",{delay: 0});
//await console.log("test email: "+number+"@gmail.com");

await page.waitForSelector('#birthDate')
await page.type("#birthDate","05/10/1990",{delay: 0});

await console.log("test email: xx "+"@gmail.com");
...
await page.waitForSelector('#permitType')
await page.click('#permitType')
await page.select('#permitType', 'Class D')
